...
| **User-suspension enforcement** | `user_suspensions` are reported to the client but **not enforced server-side** — a suspended user's still-valid access token keeps working until expiry. | Check active suspensions in `requireAuth` (or revoke token families on suspension). |
| **Rate-limit durability** | In-memory, **per-process**: behind N replicas the effective ceiling is ~N×limit; off entirely unless env-configured. Client IP trusts the first `X-Forwarded-For` hop. | Optional shared store (Redis) for cluster-wide limits; document the trusted-proxy requirement (done above). Pair with proxy/WAF limits meanwhile. |
