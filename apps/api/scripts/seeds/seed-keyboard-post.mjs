// Seed a sample "mechanical keyboard" feed post owned by the seed user, with an image uploaded through the
...
//   node scripts/seeds/seed-keyboard-post.mjs
...
// DEMO_EMAIL / DEMO_PASSWORD, KEYBOARD_IMAGE_URL.
...
const IMAGE_PATH = process.env.KEYBOARD_IMAGE_PATH || path.join(import.meta.dirname, "images", "keyboard.jpg");
const FILENAME = "keyboard.jpg";

// Original copy (not reproduced from any source) — a friendly demo post.
const TITLE = "Built my first mechanical keyboard ⌨️";
const CONTENT =
  "Hot-swap board, tactile switches, and an hour spent lubing stabilizers so the spacebar stops rattling. Worth every minute — typing now has this deep, cushioned thock that's genuinely hard to stop doing. I came for the aesthetics and stayed for the sound. ⌨️";
