// Seed a sample "monstera houseplant" feed post owned by the seed user, with an image uploaded through the
...
//   node scripts/seeds/seed-monstera-post.mjs
...
// DEMO_EMAIL / DEMO_PASSWORD, MONSTERA_IMAGE_URL.
...
const IMAGE_PATH = process.env.MONSTERA_IMAGE_PATH || path.join(import.meta.dirname, "images", "monstera.jpg");
const FILENAME = "monstera.jpg";

// Original copy (not reproduced from any source) — a friendly demo post.
const TITLE = "New monstera leaf just unfurled 🪴";
const CONTENT =
  "Woke up to a brand-new leaf slowly unrolling, and this one finally has its first real fenestrations — those signature holes. Bright indirect light, a moss pole to climb, and water only when the top inch of soil is dry. Patience really is the whole hobby. 🌿";
