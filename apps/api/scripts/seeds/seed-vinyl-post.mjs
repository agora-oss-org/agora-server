// Seed a sample "turntable setup" feed post owned by the seed user, with an image uploaded through the
...
//   node scripts/seeds/seed-vinyl-post.mjs
...
// DEMO_EMAIL / DEMO_PASSWORD, VINYL_IMAGE_URL.
...
const IMAGE_PATH = process.env.VINYL_IMAGE_PATH || path.join(import.meta.dirname, "images", "vinyl.jpg");
const FILENAME = "vinyl.jpg";

// Original copy (not reproduced from any source) — a friendly demo post.
const TITLE = "Dialing in my turntable setup 🎶";
const CONTENT =
  "Spent the afternoon balancing the tonearm — setting tracking force and anti-skate by the book instead of by vibe. The difference is wild: tighter bass, no more sibilance, and that warm crackle sitting right where it belongs. Sunday mornings belong to side A. 🎵";
