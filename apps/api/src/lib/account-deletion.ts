...
export async function requestAccountDeletion(projectId: string, profileId: string, email: string): Promise<void> {
  await getDb().delete(accountDeletionCodes)
  ...
  await getDb().insert(accountDeletionCodes).values({
...
export async function verifyAccountDeletionCode(projectId: string, profileId: string, code: string): Promise<void> {
  ...
  const [row] = await getDb().select({ id: accountDeletionCodes.id }).from(accountDeletionCodes).where(and(
  ...
  const consumed = await getDb().update(accountDeletionCodes).set({ consumedAt: new Date() })
