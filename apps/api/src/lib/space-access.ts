...
/**
 * Given a set of space ids, return the subset the caller may READ. Used to post-filter mixed result
 * sets (e.g. semantic search) without a per-record query. Operators get them all.
 */
export async function readableSpaceIds(
  c: Ctx,
  spaceIdList: Array<string | null | undefined>,
): Promise<Set<string>> {
  const ids = [...new Set(spaceIdList.filter((s): s is string => !!s))];
  const readable = new Set<string>();
  if (ids.length === 0) return readable;
  if (c.var.auth?.isOperator) {
    ids.forEach((i) => readable.add(i));
    return readable;
  }
  const uid = c.var.auth?.userId ?? null;
  const rows = await db
    .select({ id: spaces.id, userId: spaces.userId, rp: spaces.readingPermission })
    .from(spaces)
    .where(and(eq(spaces.projectId, c.var.projectId), inArray(spaces.id, ids), isNull(spaces.deletedAt)));
    ...
    const mem = await db
      ...
      .where(
        and(
          ...
          eq(spaceMembers.userId, uid),
          eq(spaceMembers.status, "active"),
          inArray(spaceMembers.spaceId, ids),
        ),
      );
    mem.forEach((m) => memberOf.add(m.spaceId));
  }
  for (const r of rows) {
    if (r.rp === "anyone" || (uid && r.userId === uid) || memberOf.has(r.id)) readable.add(r.id);
  }
  return readable;
}
