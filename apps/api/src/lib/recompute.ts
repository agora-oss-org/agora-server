  ...
  for (const id of ids) {
    ...
    const rows =
      cfg.defaultAlgorithm === "decay" && cfg.decayMode === "stored"
        ? await getDb().execute(sql`select recompute_decay_scores(${id}::uuid, ${cfg.params.halfLifeHours}::double precision, ${JSON.stringify(cfg.weights)}::jsonb) as n`)
        : await getDb().execute(sql`select recompute_scores(${id}::uuid) as n`);
