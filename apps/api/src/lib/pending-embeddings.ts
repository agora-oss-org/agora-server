...
export async function enqueuePending(
  ...
  if (cap) {
    const rows = (await getDb().execute(sql`select count(*)::int as n from pending_embeddings`)) as unknown as { n: number }[];
    if (Number(rows[0]?.n ?? 0) >= cap) {
      // At the cap: refresh an already-pending item but never grow the table.
      await getDb().update(pendingEmbeddings).set({ text, createdAt: new Date() })
  ...
  await getDb().insert(pendingEmbeddings)
...
export async function drainPendingEmbeddings(limit = 100): Promise<{ drained: number; failed: number }> {
  ...
  const rows = await getDb().select().from(pendingEmbeddings).orderBy(asc(pendingEmbeddings.createdAt)).limit(limit);
  let drained = 0;
  let failed = 0;
  for (const row of rows) {
    try {
      ...
      await getDb().delete(pendingEmbeddings)
        .where(and(eq(pendingEmbeddings.sourceType, row.sourceType), eq(pendingEmbeddings.sourceId, row.sourceId)));
      drained += 1;
    } catch (e) {
      failed += 1;
      await getDb().update(pendingEmbeddings).set({ attempts: sql`${pendingEmbeddings.attempts} + 1` })
        .where(and(eq(pendingEmbeddings.sourceType, row.sourceType), eq(pendingEmbeddings.sourceId, row.sourceId)))
        .catch(() => {});
